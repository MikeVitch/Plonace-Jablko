...
public class Stealth_Hearing : MonoBehaviour
{
    public float Detection_Rate;
    ...
    void Update()
    {
        if (Player_Seen)
        {
                stealth_enemy_logic.Alertness += Detection_Rate * Time.deltaTime;
        }
    ...
    {
        if (collision.tag == "Player_Character" && !player_movement.Is_Crouching && !Player_Behind_Wall && player_movement.Direction_Of_Movement != new Vector3(0,0,0))
        {
            Player_Seen = true;
